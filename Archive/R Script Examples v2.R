...
time_eval <- c(24, max(km.fit$time))
...
plts_pool[[i]] <- plts_pool[[i]]+
  ...
  scale_colour_manual(values =c('purple'='purple'), labels = c('Kaplan-Meier 95% CI'))
...
ggpubr::ggarrange(plts_pool[[1]]+
          ...
          labels = c("Year 2", "Year 3"),
          vjust =1,
          ncol = 2, nrow = 1,common.legend = T,legend="bottom")



ggsave(filename = paste0(pathway, "Year 2 & 3 Distributions test.png"),
       width =  9, height = 4.5)
...
m.all_expert_test <- expertsurv:::fit.models.expert(formula=Surv(time,event)~1,data=digitized_IPD,
                                               distr=c("gomp"),method="hmc",
...
max(m.all_expert_test$models[["Gompertz"]]$BUGSoutput$sims.matrix[,"St_expert[1]"])
...
m.all_expert$model.fitting



gengamma_sims <- m.all_expert$models$`Gen. Gamma`$BUGSoutput$sims.matrix
haz_mat<- apply(gengamma_sims,1, function(x){hgengamma(1:80, mu = x["mu"], sigma = x["sigma"], Q = x["Q"])})

haz_quant <- apply(haz_mat, 1, quantile, probs = c(0.025, 0.5, 0.975))
plot(1:80, y = haz_quant[2,], ylim = c(0,max(haz_quant)))
lines(1:80, y =haz_quant[1,] )
lines(1:80, y =haz_quant[3,] )
models <- names(m.all_expert$models)
psa_outuput <- list()

for(i in 1:length(m.all_expert$models)){
  psa <- make.surv(fit = m.all_expert,mod = i, nsim = n.iter, t = seq(0, 80, by = 1))
  #psa.plot(psa)
...
plt.expert <- ggplot(data = df_final_expert, aes(y = X50., x = time, group = Models, colour = Models))+
  ...
  geom_path(data = data.frame(grp = rep(times_expert[2], 10), 
   ...
   geom_segment(aes(x = 45, y = 0.73, xend = 51, yend = 0.73), color = "red")
...
summary(m.all_expert, mod = j, t = seq(0, 80, by = 1))
...
surv60_expert<- plt.expert$data %>% filter(time ==60) %>%  mutate(CredInt = X97.5.- X2.5.) %>% group_by(Models) %>% summarize(CredInt = mean(CredInt))
surv60_vague <- plt.vague$data %>% filter(time ==60) %>%  mutate(CredInt_vague = X97.5.- X2.5.) %>% group_by(Models) %>% summarize(CredInt_vague = mean(CredInt_vague))
surv_60_mods <- surv60_expert %>% select(Models, CredInt) %>% left_join(surv60_vague, by = "Models")%>% mutate(diff=CredInt_vague -CredInt ) %>% arrange(desc(diff))
...
plt.vague <- ggplot(data = df_final_vague, aes(y = X50., x = time, group = Models, colour = Models))+
  geom_line()+
  geom_line(data = df_final_vague %>% filter(Models %in% mod_select),aes(y = X97.5., x = time),linetype="dotdash")+
  geom_line(data = df_final_vague %>% filter(Models %in% mod_select),aes(y = X2.5., x = time), linetype="dotdash")+
  # geom_errorbar(data = df_final%>% filter(time == 80),aes(ymin=X2.5., ymax=X97.5.), width=8,
  #               position=position_dodge(2))+
  theme_light()+
  scale_x_continuous(expand = c(0, 0), limits = c(0,NA), breaks=seq(0, 80, 5)) + 
...
DIC_comp[order(DIC_comp$DIC_expert),]
