...
functions {
  ...
  real log_density_dist(real[ , ] params, 
    ...
    for(i in 1:7){
      if(params[i,1] == 1){
      //log_dens = exp(log_mixnorm_dens(param1, param2, param3,x))*1/7;
    }else if(params[i,1] == 2){
      
      log_dens[i] = exp(normal_lpdf(x|params[i,2], params[i,3]))*1/7;
      
    }else if(params[i,1] == 3){
      
      log_dens[i] = exp(student_t_lpdf(x|params[i,4],params[i,2], params[i,3]))*1/7;
      
    }else if(params[i,1] == 4){
      
      log_dens[i] = exp(gamma_lpdf(x|params[i,2], params[i,3]))*1/7;
      
    }else if(params[i,1] == 5){
      
      log_dens[i] = exp(lognormal_lpdf(x|params[i,2], params[i,3]))*1/7;
      
    }else if(params[i,1] == 6){
      
      log_dens[i] = exp(beta_lpdf(x|params[i,2], params[i,3]))*1/7;
...
data {
  ...
  int max_param;
  real param_expert[7,4,n_time_expert];
...
transformed parameters {
  ...
  for (i in 1:n) {
   ...
   for (i in 1:n_time_expert){
    ...
    St_expert[i] = exp(log_Sind(time_expert[i],alpha,mu[id_St[1]]));
    }else 
	  St_expert[i] = Surv_diff(alpha,mu[id_trt[1]],mu[id_comp[1]]);
...
transformed parameters {
  ...
  for (i in 1:n_time_expert){
    if(St_indic == 1){
      ...
      St_expert[i] = Surv_diff(mu[id_trt[1]],mu[id_comp[1]], alpha);  
    }
  }
...
transformed parameters {
  ...
  if(St_indic == 1){
		...
		St_expert[i] = exp(log_Sind(time_expert[i],mu[id_St[1]]));

	 }else{    
	St_expert[i] = Surv_diff(mu[id_trt[1]],mu[id_comp[1]]);
  	}
  }
}
...
transformed parameters {
  ...
  for (i in 1:n_time_expert){
    if(St_indic == 1){
      ...
      St_expert[i] = Sind(time_expert[i],alpha,mu[id_St[1]]);
