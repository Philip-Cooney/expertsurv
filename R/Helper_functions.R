...
#'                               
#' plot(example1, add.km = T, t = seq(0:20)) #Plot Survival
...
fit.models.expert <- function(formula = NULL, data, distr = NULL, method = "hmc", 
  ...
  if(!is.null(expert_type) && method == "inla"){
    print("Expert Opinion is not implemented with the inla method")
    stop()
  }
  ...
  if(!is.null(expert_type) && expert_type != "survival" && method == "mle"){
    print("Expert opinion on survival differences is not implemented for Penalized Maximum Likelihood")
    stop()
  }
