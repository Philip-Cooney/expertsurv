...
# }
# 
# alpha ~ dunif(alpha_lower, alpha_upper) #Change at the Mean Survival level
# 
# 
# for(i in 1:H){
#  beta_exp[i] ~  dunif(beta_lower[i],beta_upper[i])
#  beta[i] = log(beta_exp[i])
# }
# 
# linpred <- X %*%beta
# for( i in 1:n){
...
#     test_St_expert[i] = min((ifelse(St_expert_temp[i,1] <St_lower,1,0)+ifelse(St_expert_temp[i,1] > St_upper,1,0)),1)*St_indic
# 
# 
...
for(i in 1:n_time_expert){
St_prior[i] <- exp(-mu[id_St]/alpha * (exp(alpha*time_expert[i]) - 1))
in_bound[i] <- ifelse(St_prior[i] > St_lower,1, 0)*ifelse(St_prior[i] < St_upper,1, 0)
}

# in_bound_all<- max(prod(in_bound) + equals(St_indic,0),1) # Constraint only used for prior on survival probabilty not mean difference in survvial
# log_prior <- equals(in_bound_all,1)*100
...
alpha ~ dunif(alpha_lower_gomp, alpha_upper_gomp) #Survival must be finite for mean difference to be defined.
...
for(i in 1:n_time_expert){
St_prior[i] <- 1-pgamma(time_expert[i],alpha, lambda[id_St])
in_bound[i] <- ifelse(St_prior[i] > St_lower,1, 0)*ifelse(St_prior[i] < St_upper,1, 0)
}
...
alpha ~ dunif(alpha_lower,alpha_upper);
...
for(i in 1:n_time_expert){
St_prior[i] <- (1-pgen.gamma(time_expert[i],r,lambda[id_St],b))
in_bound[i] <- ifelse(St_prior[i] > St_lower,1, 0)*ifelse(St_prior[i] < St_upper,1, 0)
}
...
r ~ dunif(alpha_lower,alpha_upper);
b ~ dunif(alpha_lower,alpha_upper);
